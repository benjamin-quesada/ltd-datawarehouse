...
CREATE view [dbo].[sql_agent_job_details_ltd_tmdata]
as
...
from [ltd-tmdata].msdb.dbo.sysschedules 
cross apply 
...
from [ltd-tmdata].msdb.dbo.sysjobschedules as s
inner join [ltd-tmdata].msdb.dbo.sysjobs as j 
...
,h.run_duration
,h.run_status
from [ltd-tmdata].msdb.dbo.sysjobs as j 
inner join [ltd-tmdata].msdb.dbo.sysjobsteps as jb on j.job_id = jb.job_id
left join [ltd-tmdata].msdb.dbo.sysjobschedules s on s.job_id = j.job_id
left join job_schedules d on d.schedule_id = s.schedule_id
left join [ltd-tmdata].msdb.dbo.sysjobhistory h on h.job_id = jb.job_id and h.step_id = jb.step_id
