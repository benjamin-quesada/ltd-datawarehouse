...
CREATE view [dbo].[sql_agent_job_details_ltd_dw]
as
...
inner join msdb.dbo.sysjobsteps as jb on j.job_id = jb.job_id
left join msdb.dbo.sysjobschedules s on s.job_id = j.job_id
left join job_schedules d on d.schedule_id = s.schedule_id
left join msdb.dbo.sysjobhistory h on h.job_id = jb.job_id and h.step_id = jb.step_id
