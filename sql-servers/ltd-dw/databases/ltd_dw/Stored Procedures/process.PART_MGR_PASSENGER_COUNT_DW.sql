...
CREATE   PROCEDURE [process].[PART_MGR_PASSENGER_COUNT_DW]
AS
...
CHANGEDON: 
 CHANGEBY: 
   ...
   exec [process].[PART_MGR_PASSENGER_COUNT_DW]
...
SET @pscheme = 'schPASSENGER_COUNT_DW_YRMO' ;
...
SET @sqlmaxdate = 'SELECT @maxdate = MAX([CalendarPartitionKey]) FROM [tm].[PASSENGER_COUNT_DW]' ; 

EXEC sp_executesql @sqlmaxdate
 , N'@maxdate NUMERIC(11,0) OUTPUT'
 , @maxdate = @maxdate OUTPUT ;

SELECT @currentpartition = $PARTITION.[pfPASSENGER_COUNT_DW_YRMO](@maxdate) ;
...
ELSE 
 ...
 SET @sql = 'ALTER PARTITION SCHEME schPASSENGER_COUNT_DW_YRMO 
 NEXT USED [PRIMARY] 
 
 ALTER PARTITION FUNCTION pfPASSENGER_COUNT_DW_YRMO() SPLIT RANGE (''' + CONVERT( VARCHAR(10), @partvalue, 112 ) + ''')' ;  --Range Partition Value
