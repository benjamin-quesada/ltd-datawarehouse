...
CREATE   PROCEDURE [tm].[view_store_pass_exception]
...
purpose	 :  Add object activities on who, what, when call this object
			...
			exec tm.view_store_pass_exception
...
IF (SELECT COUNT(*) FROM ##viewstorepassx) > 0
BEGIN
...
             ,@recipients = 'barb.eichberger@ltd.org;Sopheap.suy@ltd.org;data@ltd.org' 
             ,@subject = 'review: Possible missing data for passenger count: Attachment'
			 ,@query = 'select * from ##viewstorepassx ORDER BY 1,2,3'
			 ,@attach_query_result_as_file = 1
             ,@body = 'review: Possible missing data for passenger count. Attachment(1)


'
END

IF (SELECT COUNT(*) FROM tempdb.sys.tables WHERE name LIKE '%viewstorepassx%') > 0
BEGIN
	DROP TABLE ##viewstorepassx
END
