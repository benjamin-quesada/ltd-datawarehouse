...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [eam].[get_part_movement_min_max_wo_eq]
AS
...
UPDATED BY	: B. Eichberger
UPDATED DT	: 3/12/2025
purpose		: RID-30426 BI Min Max Report: Add column for part no yymm

USE			: exec [eam].[get_part_movement_min_max_wo_eq]
----------------------------------*/
...
SELECT *
INTO #on_order
FROM Reporting.eam.parts_on_order

part_no_ordered_yyyymm


SELECT * INTO #on_hand
FROM Reporting.[eam].[parts_on_hand_current]
...
,c.qty_on_hand
,o.qty_on_order
,[On Hand Plus On Order] = c.qty_on_hand + o.qty_on_order
,i.SafetyStockLevel
...
LEFT JOIN #on_order o ON o.part_no_ordered_yyyymm = i.part_no_yyyymm
LEFT JOIN #on_hand c ON c.part_no_suffix COLLATE SQL_Latin1_General_CP1_CI_AS = i.part_no_suffix COLLATE SQL_Latin1_General_CP1_CI_AS

--SELECT * FROM #mergeReady WHERE PART_part_no = '6224-0177' ORDER BY count_year, count_month

UPDATE d 
SET d.qty_on_order = ISNULL(m.qty_on_order,0)
,d.TotalQuantityOnOrderForAllLoc = ISNULL(m.qty_on_order,0)
FROM #mergeReady d
JOIN (SELECT part_no_suffix,isnull(MAX(qty_on_order),0) qty_on_order FROM #mergeReady 
		GROUP BY part_no_suffix) m ON m.part_no_suffix = d.part_no_suffix 



TRUNCATE TABLE 
[eam].[parts_movement_min_max_wo_equip_no] 


INSERT [eam].[parts_movement_min_max_wo_equip_no]
(PrimaryBinID
,IsObsolete
,IsTBD
,PART_part_no
...
OUTPUT inserted.[part_min_max_key_noeq] INTO @outputTbl
