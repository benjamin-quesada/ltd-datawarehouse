...
CREATE   PROCEDURE [rpt].[transTrack_Sched_vs_Actual_Miles_and_Minutes] as
...
/* 
  PURPOSE: maintains data populated in rpt.[transTrack_Sched_vs_Actual_Miles_Time] - Prepared data for transtrack.
   ...
   exec [rpt].[transTrack_Sched_vs_Actual_Miles_and_Minutes] 
...
declare @stdt datetime2 = '7/1/2019' --(select isnull(min(stdt),'7/1/2020') stdt from (
							--select max(record_created_date) stdt from rpt.[transTrack_Sched_vs_Actual_Miles_Time] WITH (NOLOCK)
							--union 
							--select max(record_updated_date) from rpt.[transTrack_Sched_vs_Actual_Miles_Time] WITH (NOLOCK)) o )
declare @stdtint int = (select convert(varchar(32), @stdt, 112) + 100000000)


declare @lastdt date 
 select @lastdt = '7/31/2021' -- (select isnull(CONVERT(date,cast((max(calendar_id)-100000000) as char(10)),120),DATEADD(DAY,-2,GETDATE())) from [ltd-tmdata].[ltd_db].[dbo].[ntd_stats_transtrack])
...
select [calendar_id]
	  ...
	  ,svc = case when st.SERVICE_TYPE_TEXT like '%w%' then 'Wkd' else left(st.SERVICE_TYPE_TEXT, 3) end
	   ...
	   FROM          [ltd-tmdata].tmdatamart.dbo.schedule				   s WITH (NOLOCK)
		   left join [ltd-tmdata].tmmain.dbo.trip				           t WITH (NOLOCK)  on t.trip_id             = s.trip_id
		   left join [ltd-tmdata].tmdatamart.dbo.service_type             st WITH (NOLOCK)  on st.service_type_id    = s.service_type_id
   where calendar_id = 120200701 -- @currdtInt -- 
...
select [calendar_id] 
 ...
 group by [calendar_id] ,time_table_version_id
	  ,vehicle_id
      ,route_id
	  ,route_direction_id
      ,pattern_id  
...
insert [rpt].[transTrack_Sched_vs_Actual_Miles_Time] (
	   [calendar_id]
...
AND NOT EXISTS (SELECT 1 FROM [rpt].[transTrack_Sched_vs_Actual_Miles_Time] x
