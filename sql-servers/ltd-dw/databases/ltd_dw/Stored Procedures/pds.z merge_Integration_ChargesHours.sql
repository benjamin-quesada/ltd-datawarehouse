...
CREATE   PROCEDURE [pds].[z merge_Integration_ChargesHours]
AS
/*-----------LTD_GLOSSARY---------------
...
DECLARE @lastDate DATE = (SELECT DATEADD(MONTH,-2,MAX(CAST([charge_date] AS DATE))) FROM [pds].[Integration_ChargesHours])
...
,ISNULL(s.[charge_date],'1/1/1900')[charge_date]
,ISNULL(s.[charge_hours],'')[charge_hours]
,ISNULL(s.[charge_amount],'')[charge_amount]
,ISNULL(s.[context_user_id],'')[context_user_id]
,ISNULL(s.[ph_id]	   ,'')[ph_id]
,ISNULL(s.[phcntl_id]  ,'')[phcntl_id]
,ISNULL(s.[name]	   ,'')[name]
...
,ISNULL(s.[aka]		   ,'')[aka]
,ISNULL(s.[employee_id],'')[employee_id]
,ISNULL(s.[company_code],'')[company_code]
,ISNULL(s.[company]	   ,'')[company]
,ISNULL(s.[check_number],'')[check_number]
,ISNULL(s.[check_date] ,'1/1/1900')[check_date]
,ISNULL(s.[period_end_date],'1/1/1900')[period_end_date]
,ISNULL(s.[charge_seq] ,'')[charge_seq]
...
,ISNULL(s.[charge_shift_amount],'')[charge_shift_amount]
,ISNULL(s.[charge_rate],'')[charge_rate]
,ISNULL(s.[charge_base_rate],'')[charge_base_rate]
,ISNULL(s.[charge_percent],'')[charge_percent]
,ISNULL(s.[is_auto_gen],'')[is_auto_gen]
,ISNULL(s.[shift_name] ,'')[shift_name]
,ISNULL(s.[is_overtime],'')[is_overtime] -- 
 INTO #tempStage 
FROM [pds].[Integration_ChargesHours_Stage] s
WHERE ISNULL(s.[charge_date],'1/1/1900') >= @lastDate
...
MERGE [pds].[Integration_ChargesHours] AS t
...
WHEN MATCHED AND ( 
    t.[charge_number]   <> s.[charge_number]
 OR t.[context_user_id] <> s.[context_user_id]
 OR t.[ph_id]			<> s.[ph_id]
 ...
 OR t.[charge_shift_amount] <> s.[charge_shift_amount] 
 OR t.[charge_rate]		<> s.[charge_rate] 
 OR t.[charge_base_rate]<> s.[charge_base_rate] 
 OR t.[charge_percent]	<> s.[charge_percent] 
 OR t.[shift_name]		<> s.[shift_name]	 
 OR t.[location_code] <>	s.[location_code]
 OR t.[distribution_code] <> s.[distribution_code]
 OR t.[is_auto_gen] <> s.[is_auto_gen]
...
,s.[shift_name] 
,s.[is_overtime])
WHEN NOT MATCHED BY SOURCE AND CAST([charge_date]AS DATE) >= @lastDate THEN DELETE
