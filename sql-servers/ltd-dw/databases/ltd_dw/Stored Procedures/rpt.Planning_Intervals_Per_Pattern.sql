...
CREATE     PROCEDURE [rpt].[Planning_Intervals_Per_Pattern]
@p_ttv NVARCHAR(32) ,
@p_rte NVARCHAR(32) ,
@p_rdir NVARCHAR(1) 
AS
...
GRANT EXECUTE ON [rpt].[Planning_Intervals_Per_Pattern] to public
exec rpt.[Planning_Intervals_Per_Pattern] '2202a','51', 'I'
...
DECLARE @rdirSelect NVARCHAR(90) = (SELECT CASE WHEN @p_rdir = 'All' OR ISNULL(@p_rdir,'') = '' THEN '' ELSE ' AND left(rd.route_direction_name, 1) = ''' + @p_rdir +'''' END )
...
from        [ltd-tmdata].tmmain.dbo.pattern                   p
 inner join [ltd-tmdata].tmdatamart.dbo.time_table_version        ttv on ttv.time_table_version_id = p.time_table_version_id
 inner join [ltd-tmdata].tmdatamart.dbo.[route]                   r   on p.route_id                = r.route_id
 inner join [ltd-tmdata].tmdatamart.dbo.route_direction           rd  on rd.route_direction_id     = p.route_direction_id
