...
CREATE   PROCEDURE [rpt].[TouchPassDataforImport_NP_v2] 
@yyyymm varchar(32) ,
...
grant execute on [rpt].[TouchPassDataforImport_NP_v2]  to rpt_reader

AUTHOR:  B. EICHBERGER
CREATED: 20200720
TICKET:  RID 10100

example: exec rpt.[TouchPassDataforImport_NP] '202107',.75
...
BEGIN
DROP TABLE #TPsalesDetail_NP
END

if (select count(*) from tempdb.sys.tables where name like '%TPdiscLines_NP%') <>0
BEGIN
DROP TABLE #TPdiscLines_NP
END

if (select count(*) from tempdb.sys.tables where name like '%TPsalesDetail_NP%') =0
BEGIN
CREATE TABLE #TPsalesDetail_NP(
...
select cust_no
     ...
     , gcpro
     , RevCode
     , acctcode from #TPdiscLines_NP
) o
order by 2,1,17


--select * from #TPdiscLines_NP where cust_no in ('0106','0163','0168')
--select * from #TPsalesDetail_NP where cust_no in ('0106','0163','0168')
