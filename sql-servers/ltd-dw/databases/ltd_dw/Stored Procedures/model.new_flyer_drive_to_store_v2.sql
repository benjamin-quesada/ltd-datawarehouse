...
CREATE   PROCEDURE [model].[new_flyer_drive_to_store_v2]
AS

-- exec [model].[new_flyer_drive_to_store_v2]
...
FROM (
	...
	FROM (
		SELECT [drive_id]
				  ...
				  ,[end_longitude]
				  ,[distance]
				  ,[drive_duration] = CAST(dbo.[ConvertTimeToHHMMSS](DATEDIFF(MINUTE,CAST([start_time_local] AS DATETIME2),CAST([end_time_local] AS DATETIME2)),'second') AS TIME(0))
				  ,drive_duration_seconds = DATEDIFF(SECOND,CAST([start_time_local] AS DATETIME2),CAST([end_time_local] AS DATETIME2))
				  
				  --,[drive_duration] = datediff(second,cast([start_time] AS DATETIME2) ,cast([end_time] AS datetime2))
				  --,drive_duration_seconds = datediff(second,cast([start_time] AS DATETIME2) ,cast([end_time] AS datetime2))
				  ,CAST([start_time_local] AS DATETIME2) start_time
				  ,CAST([end_time_local] AS DATETIME2) end_time
				  ,trip_start_spm = DATEDIFF(SECOND, 0,DATEADD(DAY, 0 - DATEDIFF(DAY, 0, [start_time_local]), [start_time_local]))
				  ,trip_end_spm = DATEDIFF(SECOND, 0,DATEADD(DAY, 0 - DATEDIFF(DAY, 0, [p].[end_time_local]), [end_time_local]))
				  ,trip_start_calendar_id = CAST(CONVERT(VARCHAR(32),CAST([start_time_local] AS DATE),112) AS INT) + 100000000
				  ,trip_end_calendar_id = CAST(CONVERT(VARCHAR(32),CAST([end_time_local] AS DATE),112) AS INT) + 100000000
			 FROM [ltd_dw].[nf].[newflyer_trips]
			 --[dbo].[newflyer_vehparams] 
			 p WITH (NOLOCK)
		...
		SELECT [drive_id]
				  ...
				  ,CAST([end_time_local] AS DATETIME2) end_time
				  ,trip_start_spm = DATEDIFF(SECOND, 0,DATEADD(DAY, 0 - DATEDIFF(DAY, 0, [start_time_local]), [start_time_local]))
				  ,trip_end_spm = DATEDIFF(SECOND, 0,DATEADD(DAY, 0 - DATEDIFF(DAY, 0, [end_time_local]), [end_time_local]))
				  ,trip_start_calendar_id = CAST(CONVERT(VARCHAR(32),CAST([start_time_local] AS DATE),112) AS INT) + 100000000
				  ,trip_end_calendar_id = CAST(CONVERT(VARCHAR(32),CAST([end_time_local] AS DATE),112) AS INT) + 100000000
			 FROM [ltd_dw].[nf].[newflyer_trips] p WITH (NOLOCK)
			 WHERE ISNULL(start_latitude,0) <> 0
			   AND CAST([start_time_local] AS DATETIME2) >= '5/1/2021'
			   AND distance > 0
			   AND license_number IS NOT NULL
			   --AND drive_id = 71352566329 -- 71352566329
			 GROUP BY 
...
WHERE NOT EXISTS (SELECT 1 FROM model.newflyer_drive_store 
					where drive_id = q.drive_id
					  and cast(trip_start_calendar_id AS VARCHAR(32)) + RIGHT('000000'+ CAST(trip_start_spm AS VARCHAR(32)),6) = start_spm_key
					  and cast(trip_end_calendar_id AS VARCHAR(32)) + RIGHT('000000'+ CAST(trip_end_spm AS VARCHAR(32)),6) = end_spm_key 
					  and license_number = q.[license_number]
					  )
GROUP BY 
[start_spm_key]
      ,[end_spm_key]
