...
CREATE   PROCEDURE [pds].[merge_Integration_EmpAbsent_Hist]
AS
/*-----------LTD_GLOSSARY---------------
...
DECLARE @lastProcess smalldatetime = (SELECT DATEADD(DAY,-30,MAX([MergeBeginDatetime]))
										...
										and ObjectProgram like 'SSIS_ISC_PROCESS_Integration_EmpAbsentHistory%')
...
INTO #empAbsent_stage
  ...
  where cast(from_date as smalldatetime) >= @lastProcess
...
ON (
	   ...
	   AND t.from_date = cast(s.from_date as date)
   )
WHEN MATCHED AND ( 
...
WHEN NOT MATCHED BY SOURCE and t.from_date >= @lastProcess THEN DELETE
...
DECLARE @prg varchar(90) = @@SERVERNAME + '.ltd_dw.pds.merge_Integration_EmpAbsent_Hist: ' + CAST(@allCount AS VARCHAR(12))
