...
CREATE    PROCEDURE [process].[Eden_Extract_AR_step2]
...
-- Description:	Process Process Eden Extract AR for finance
...
DECLARE @isRunning INT = (
	...
	FROM
		...
		, GETDATE()) AS Elapsed, activity.stop_execution_date
			 ...
			 WHERE run_requested_date IS NOT NULL AND stop_execution_date IS NULL AND job.name='Eden Extract AR'
		) i	
	) 
IF @isRunning = 0 
BEGIN
...
EXEC [LTD-FINANCE].msdb.dbo.sp_start_job @job_name='Eden Extract AR'


SELECT '
As requested by '+REPLACE(@fixedUser,'LTD\','')+', the Eden AR Transactions Processor has started.
...
The Eden AR Transactions Processor job is alraedy running. Please come back later.
