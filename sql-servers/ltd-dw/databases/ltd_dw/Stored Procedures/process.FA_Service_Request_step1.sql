...
purpose	 :  modify logic for usernames (null resolves to null, single name resolves to single name, full name resolves to first name)
			updated email for notifications

=============================================
*/


CREATE     PROCEDURE [process].[FA_Service_Request_step1]
...
-- DROP TABLE IF EXISTS process.JobStepDataFAServiceReq
	...
	DECLARE @full_name_of_user AS VARCHAR(42) = REPLACE(@fixedUser, 'LTD\', '')

	SELECT @fasrkeynumber AS fasrkeynbr,'
Greetings ' + LEFT(@full_name_of_user, ISNULL(NULLIF(CHARINDEX(' ', @full_name_of_user),0),9999)) + ',
