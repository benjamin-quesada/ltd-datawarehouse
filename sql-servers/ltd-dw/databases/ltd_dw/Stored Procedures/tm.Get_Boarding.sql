...
CREATE   PROCEDURE [tm].[Get_Boarding]
as
-- exec tm.Get_Boarding

/*
  CREATED: 20190829
   AUTHOR: B EICHBERGER
  PURPOSE: Collect Daily BOarding Status data for longitudinal record in DW - WIP.
CHANGEDON: 20200121
...
select isnull(max(calendar_id),@backupdate) maxdate
 from [tm].[BoardingStats] WITH (NOLOCK)

update [process].[MergeLogs] 
set[MergeEndDatetime] = @startdt
	,recInsert = 0
	,recDelete = 0
	,recUpdate = 0 
   where mergecode = 'PASS'
     and [ObjectDestination] = 'LTD_DW.tm.BoardingStats'
	 AND [ObjectSource] = 'TM'
	 AND [ObjectProgram] = 'LTD_DW.tm.Get_Boarding'
	 AND [MergeEndDatetime] is null
	 --AND MergeBeginDatetime = 
 ...
 insert [process].[MergeLogs] (
      ...
      ,[MergeBeginDatetime])
	  Values(
	  'PASS', 'LTD_DW.tm.BoardingStats','TM','LTD_DW.tm.Get_Boarding',0, 0, 0, @startdt)


select *,cast((CONVERT(DECIMAL(10, 6), maplat * 0.0000001)) as varchar(32))+' '+cast((CONVERT(DECIMAL(10, 6), maplong * 0.0000001)) as varchar(32)) as LatLong
...
select [calendar_id]            = lm.calendar_id
      ...
      ,passenger_count_on       = sum(lm.passenger_count_on)
      ,passenger_count_off       = sum(lm.passenger_count_off)
...
-- select top 100 *
 ...
 where (CONVERT(DECIMAL(10, 6), lm.[latitude] * 0.0000001)) not between 44.042901 - .001 and 44.042901 + .001
	...
	and lm.direction <> 0
	and (passenger_count_on is not null or passenger_count_off is not null)
...
MERGE tm.BoardingStats as t
 USING #BoardingSource as s on 
...
,s.[stop_code]
,s.[stop_name]
,s.[passenger_count_on]
,s.[passenger_count_off]
,s.[valid_odometer]
,s.[valid_position]
,s.[east_west_zone]
,s.[north_south_zone]
...
,s.[LatLong])
WHEN MATCHED AND
 isnull(s.[passenger_count_on],0) <> isnull(t.[passenger_count_on],0)
OR isnull(s.[passenger_count_off],0) <> isnull(t.[passenger_count_off],0)
OR isnull(s.[valid_odometer],0) <> isnull(t.[valid_odometer],0)
OR isnull(s.[valid_position],'') <> isnull(t.[valid_position],'')
THEN UPDATE
set t.[passenger_count_on] = s.[passenger_count_on]
, t.[passenger_count_off] = s.[passenger_count_off]
