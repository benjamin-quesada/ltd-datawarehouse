...
CREATE   PROCEDURE [rpt].[Get_VOMS_DAILY_HOURLY_RS]
@stDate INT,
@toDate INT
AS
...
EXEC EXAMPLE: exec rpt.Get_VOMS_DAILY_HOURLY_RS 20190101, 20200831
...
and MergeCode = 'VOMS'
and [ObjectDestination] = 'ltd_dw.rpt.VOMS_DAILY_HOURLY_RS'
...
insert ltd_dw.[process].[MergeLogs] (
      ...
      ,[MergeBeginDatetime])
	  Values(
	  'VOMS', 'ltd_dw.rpt.VOMS_DAILY_HOURLY_RS','NOVUS','ltd_dw.rpt.Get_VOMS_DAILY_HOURLY_RS',0, 0, 0, @workstartdt)
...
select * 
into #RSpassbk 
from [LTD-DW2].[Novus_PROD].dbo.PassBooking pb WITH (NOLOCK)
...
left(CAST(DATEADD(SECOND,n,0) AS TIME),8) HHMMSS,
 count(distinct runId) current_runs 
...
and s.n <= dropOffTime 
 ...
 MERGE [rpt].[VOMS_DAILY_HOURLY_RS] t
...
,[MergeEndDatetime] = SYSDATETIME()
   WHERE mergecode = 'VOMS'
	 ...
	 AND [ObjectProgram] = 'ltd_dw.rpt.Get_VOMS_DAILY_HOURLY_RS'
