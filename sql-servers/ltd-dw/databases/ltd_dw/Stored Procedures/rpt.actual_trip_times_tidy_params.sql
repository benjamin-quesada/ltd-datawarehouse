...
CREATE   PROCEDURE [rpt].[actual_trip_times_tidy_params] 
@startDate date ,@endDate date, @rte varchar(12) null,@trip_end_act varchar(12) null,@trip_end_sch varchar(12) null,@srv varchar(12) null,@bid varchar(12) null
...
Permitted	: grant execute on [rpt].[actual_trip_times_tidy_params] to rpt_reader
			  grant execute on [rpt].[actual_trip_times_tidy_params] to "LTD\LTD_Data"
*/
/*------------------LTD_GLOSSARY---------------
...
if (isnull(@startDate,'1/1/1900') = '1/1/1900' OR (isnull(@endDate,'1/1/1900') = '1/1/1900'))
BEGIN
RETURN
END
...
select calendar_id = a_ts.calendar_id
      ...
      from      [ltd-tmdata].tmdatamart.dbo.adherence    a_ts WITH (NOLOCK)
 inner join [ltd-tmdata].tmmain.dbo.time_table_version   ttv WITH (NOLOCK)   on ttv.time_table_version_id = a_ts.time_table_version_id
 inner join -- select * from 
 [ltd-tmdata].ltd_db.dbo.ltd_service_calendar_from_tmmain sc WITH (NOLOCK)    on sc.calendar_id            = a_ts.calendar_id
 inner join [ltd-tmdata].tmmain.dbo.[route]              rte WITH (NOLOCK)   on rte.route_id              = a_ts.route_id
 inner join [ltd-tmdata].tmmain.dbo.route_direction      rd WITH (NOLOCK)    on rd.route_direction_id     = a_ts.route_direction_id
 inner join [ltd-tmdata].tmmain.dbo.time_point           tp_ts WITH (NOLOCK) on tp_ts.time_point_id       = a_ts.time_point_id
 inner join [ltd-tmdata].tmmain.dbo.trip                 t WITH (NOLOCK)     on t.trip_id                 = a_ts.trip_id
 inner join [ltd-tmdata].tmmain.dbo.pattern              p WITH (NOLOCK)     on p.pattern_id              = t.pattern_id
 inner join [ltd-tmdata].tmdatamart.dbo.adherence        a_te WITH (NOLOCK)  on a_te.calendar_id          = a_ts.calendar_id and a_te.trip_id = a_ts.trip_id and a_te.scheduled_time = t.trip_end_time and a_te.overload_id = a_ts.overload_id
 inner join [ltd-tmdata].tmmain.dbo.time_point           tp_te WITH (NOLOCK) on tp_te.time_point_id       = a_te.time_point_id
 where isnumeric(left(rte.route_abbr, 1)) = 1
 AND a_ts.calendar_id between @calidstart and @calIdEnd
 AND (@bid IS NULL OR (sc.bid = @bid))
 AND (@rte IS NULL OR (rte.route_abbr  = @rte ))
 AND (@trip_end_act IS NULL OR (tm.convert_spm_to_hh_mm_ss(a_te.actual_arrival_time) = @trip_end_act    ))
 AND (@trip_end_sch IS NULL OR (tm.convert_passing_time(t.trip_end_time) = @trip_end_sch    ))
 AND (@srv IS NULL OR (tm.convert_passing_time(sc.srv_gen) = @srv    ))
   ...
   -- @startDate date null ,@endDate date null, @rte varchar(12) null,@trip_end varchar(12) null,@srv varchar(12) null,@bid varchar(12) null
