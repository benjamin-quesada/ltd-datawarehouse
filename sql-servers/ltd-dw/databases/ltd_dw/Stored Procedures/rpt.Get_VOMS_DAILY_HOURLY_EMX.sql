...
CREATE   PROCEDURE [rpt].[Get_VOMS_DAILY_HOURLY_EMX]
@stDate INT,
@toDate INT,
@busType VARCHAR(12)
as
...
EXEC EXAMPLE: exec rpt.Get_VOMS_DAILY_HOURLY_EMX 20200101, 20200111, 'emx'
...
and MergeCode = 'VOMS'
and [ObjectDestination] = 'ltd_dw.rpt.VOMS_DAILY_HOURLY_EMX'
...
insert ltd_dw.[process].[MergeLogs] (
      ...
      ,[MergeBeginDatetime])
	  Values(
	  'VOMS', 'ltd_dw.rpt.VOMS_DAILY_HOURLY_EMX','NOVUS','ltd_dw.rpt.Get_VOMS_DAILY_HOURLY_EMX',0, 0, 0, @workstartdt)
...
INSERT INTO -- select * from 
 #LTDVOMSData  
SELECT trip_id, block_id, calendar_id, vehicle_id
...
select calendar_id,left(HHMMSS,2) HH,max(current_runs) current_runs 
into #LTDvomssource
from (	
select calendar_id,n,
left(CAST(DATEADD(SECOND,n,0) AS TIME),8) HHMMSS,
 count(distinct trip_id) current_runs 
...
and s.n <= lastDropOffTime 
 ...
 MERGE [rpt].[VOMS_DAILY_HOURLY_EMX] t
...
,[MergeEndDatetime] = sysdatetime()
   where mergecode = 'VOMS'
	 ...
	 AND [ObjectProgram] = 'ltd_dw.rpt.Get_VOMS_DAILY_HOURLY_EMX'
