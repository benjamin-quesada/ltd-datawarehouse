CREATE TABLE [eam].[parts_fuel_inventory_fom]
(
...
[record_created_date] [datetime2] NOT NULL CONSTRAINT [DF__parts_fue__recor__60C42615] DEFAULT (sysdatetime())
) ON [PRIMARY]
GO
ALTER TABLE [eam].[parts_fuel_inventory_fom] ADD CONSTRAINT [PK_parts_fuel_inventory] PRIMARY KEY CLUSTERED ([PFDI_Key]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [ix_parts_fuel_inventory_fom_Calendar_Id] ON [eam].[parts_fuel_inventory_fom] ([Calendar_Id]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [ix_parts_fuel_inventory_fom_InvYear] ON [eam].[parts_fuel_inventory_fom] ([InvYear]) INCLUDE ([InvMonth]) ON [PRIMARY]
GO
