...
PURPOSE		: for local report usefulness for fuel ticket and service polygons
USE			: exec [tm].[get_logged_message_from_tmdailylog]
...
DECLARE @lastTMID BIGINT = (SELECT MAX([transmitted_message_id]) FROM ltd_dw.tm.logged_messages) 
...
INTO #tempLoggMess
 ...
 WHERE [transmitted_message_id] >= @lastTMID
...
DECLARE @prg varchar(90) = @@SERVERNAME + '.ltd_dw.tm.get_logged_message_from_tmdailylog'
...
DROP TABLE IF EXISTS #tempLoggMess
