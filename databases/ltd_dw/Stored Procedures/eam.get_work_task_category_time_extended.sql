...
CREATE   PROCEDURE [eam].[get_work_task_category_time_extended]
AS
-- exec eam.get_work_task_category_time_extended
...
CREATED		20240725
AUTHOR		S Suy
PURPOSE		Optimize and change to incremental type loader

-- GRANT SELECT on [eam].[workOrderTaskCategoryTimeExtended] to rpt_reader
...
declare @process_date DATETIME = '2026-02-18'
...
where fully_reversed = ''N''
   ...
   and 100000000+ cast(convert(varchar(32),(cast(L.[X_datetime_insert] as date)),112) as INT) >= ''120120701''
...
wo_task_inserted = tm.X_datetime_insert
                ...
                ,wo_task_yr_no = cast(tm.work_order_yr AS VARCHAR(8)) + '-' + cast(tm.work_order_no AS VARCHAR(7))
...
CREATE TABLE wrk.NEWworkOrderDW(
                ...
                CLASS_class_maint VARCHAR(32) NULL,
                CLASS_class_stds VARCHAR(32) NULL,
                ...
                eq_equip_no VARCHAR(32) NULL,
                life_miles INT NULL,
                ltd_bus_class VARCHAR(32) NOT NULL,
                ...
                ltd_bus_class_adj VARCHAR(32) NULL,
...
SELECT work_order_yr
	...
	-- select top 10 * 
FROM #stageWorkOrder
WHERE NOT (work_order_no = 2 AND work_order_yr = 2013)
GROUP BY
	work_order_yr
	,work_order_no
	,calendar_id
...
merge -- truncate table
	eam.workOrderTaskCategoryTimeExtended AS t
...
VALUES
(s.calendar_id, s.work_order_yr, s.work_order_no, s.estimate, s.job_type, s.comml_cost, s.warranty
, s.account_id, s.indirect_flag, s.wcl_work_class, s.labor_cost_job, s.labor_cost_lab
, s.work_order_status, s.datetime_out_service, s.datetime_in_service, s.datetime_closed, s.datetime_unit_in
, s.qty_est_hours, s.meter_1_life_total, s.meter_1_reading, s.TASK_task_code, s.CLASS_class_maint, s.CLASS_class_stds
, s.date_entered, s.lab_start_time, s.lab_end_time, s.task_type, s.task_reason, s.emp_empl_no, s.labor_rate, s.labor_hours, s.jobmain_labor_hours, s.labmain_labor_hours, s.labor_date, s.wo_task_inserted, s.wo_task_calendar_id, s.wo_task_yr_no, s.employee_name, s.eq_equip_no, s.life_miles, s.ltd_bus_class, s.bio_diesel, s.atric, s.emx_bus, s.hybrid, s.electric, s.max_fuel, s.active, s.unit_is_active, s.ltd_bus_class_adj, s.repair_group, s.repair_group_code, s.repair_category, s.backupCategory, s.task_code, s.category
, s.row_id, s.group_row_id
)
...
DECLARE @prg varchar(90) = @@SERVERNAME + '.ltd_dw.eam.workOrderTaskCategoryTimeExtended'
...
(		[MergeCode]
           ...
           ,[MergeEndDatetime])
SELECT 'WRKOR',
'ltd_dw.eam.workOrderTaskCategoryTimeExtended',
'EAM',
@prg,
