...
CREATE    PROCEDURE [process].[Eden_Extract_GJ_step1]  @currentUser VARCHAR(42)

AS
-- ================LTD_GLOSSARY=============================
-- Author:		B Eichberger
-- Create date: 2024020t
-- Description:	Process Eden Extract GJ for finance
-- example:		exec process.Eden_Extract_GJ @currentUser	
...
SELECT SUM(rc) FROM 	
	...
	FROM  
		...
		, GETDATE()) AS Elapsed, activity.stop_execution_date
			 ...
			 WHERE activity.run_requested_date IS NOT NULL AND activity.stop_execution_date IS NULL AND job.name='Eden Extract GJ'
			 ) i
		) t 
	)
IF @isRunning = 0 
BEGIN
	...
	DROP TABLE IF EXISTS process.JobStepStateEdenGJ;
	CREATE TABLE process.JobStepStateEdenGJ
	(
	...
	GRANT SELECT ON process.JobStepStateEdenGJ TO PUBLIC;
	INSERT INTO process.JobStepStateEdenGJ (Job_Name, VariableName, VariableValue)
	VALUES ('Eden Extract GJ','UserFromPBIRS', @fixedUser
	);
...
Click here to start Processing Eden Extract GJ.
...
The Eden Extract GJ job is running right now. Please come back later.
 '
 AS banner
END 
