...
FROM (SELECT DISTINCT file_row_id, tpat_route, tpat_external_id, tpat_direction, tpatpt_stop_id, tpatpt_load_place FROM hastus.avl_pat 
      WHERE ISNUMERIC(tpat_route)=1 ) g
...
SELECT rn = ROW_NUMBER() OVER (ORDER BY u.tpat_route),tpat_route INTO #rte_loop 
...
SELECT DISTINCT
    ...
    CASE WHEN tpatpt_is_timing_point = 'X' THEN file_row_id ELSE NULL END AS ParentGroupID
INTO #tp_places
FROM #pat t
...
SELECT 
    ...
    MAX(ParentGroupID) OVER (ORDER BY file_row_ID ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS EffectiveParentID
FROM #tp_places l 
