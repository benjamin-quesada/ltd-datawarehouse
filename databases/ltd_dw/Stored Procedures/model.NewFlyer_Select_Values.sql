...
CREATE PROCEDURE [model].[NewFlyer_Select_Values] (
...
USAGE	  :		exec nf.prepare_data_for_cte 120211012 (parameter presented by loop programming 
				...
				-- exec [nf].[prepare_data_for_cte_v2] '2/28/2022', 20201	
*/
...
--DECLARE @licCurr INT = 20211
DECLARE @startdateint INT = 120220323 -- CAST(CONVERT(VARCHAR(32),@startDateDt,112) AS INT)+100000000
DECLARE @endDateInt INT = 120220324 --(SELECT TOP(1) CONVERT(varchar(32),GETDATE()-2,112)+100000000 ORDER BY 1)
DECLARE @min bigint, @max bigint

--SELECT 24*60*60
SELECT @Min=0 ,@Max=86399;
;
...
FOR parameter_type IN 
	   ...
	   , [39223], [10004], [13068], [13073], [50105], [49823], [49838], [49839]
		)
) AS pvt
GROUP BY
