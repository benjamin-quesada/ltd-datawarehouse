...
CREATE PROCEDURE [tm].[merge_ONTIME_PERFORMANCE_wo_arr_ann_stations]
AS
/*-----------LTD_GLOSSARY---------------
 created by	:  b. eichberger
 created dt	:  2025-11-21
 purpose	:  merge tm.ONTIME_PERFORMANCE from ltd-tmdata.tmmain.dbo.ONTIME_PERFORMANCE_wo_arr_ann_stations
			   to demonstrate differences when excluding specific stop types and stations

	use		:  exec [tm].[merge_ONTIME_PERFORMANCE_wo_arr_ann_stations]
...
SELECT * 
INTO #ot60
FROM ltd_dw.[tm].[ontime_performance_v_wo_ann_arr_stations]
WHERE the_date >= DATEADD(DAY, -60, GETDATE())


MERGE ltd_dw.tm.ONTIME_PERFORMANCE_wo_arr_ann_stations AS t
USING #ot60 AS s
ON (t.svc = s.svc
...
DECLARE @prg VARCHAR(90) = @@SERVERNAME + '.ltd_dw.tm.merge_ONTIME_PERFORMANCE_wo_arr_ann_stations'
...
(		[MergeCode]
           ...
           ,[MergeEndDatetime])
SELECT 'OTP',
'ltd_dw.tm.ONTIME_PERFORMANCE_wo_arr_ann_stations',
'TMDM',
@prg,
