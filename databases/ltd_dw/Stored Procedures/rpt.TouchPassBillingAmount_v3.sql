...
CREATE   PROCEDURE [rpt].[TouchPassBillingAmount_v3]
as
/*
...
truncate table -- select * from 
stg.TouchPassSalesAmount
...
--SELECT *	 FROM tpb.[202304 Billing]

select *, FLOOR(cn/10)*10 bucket
into -- SELECT * FROM 
#rowiterator
FROM 	
(
...
declare @currentTbl nvarchar(120) = ''
...
DROP TABLE IF EXISTS dbo.#buckets
...
INSERT stg.TouchPassSalesAmount
...
UNPIVOT(AmtSales FOR Product IN (' + 
@columnstmnt
+')) AS unpvt
WHERE AmtSales <> 0
and len([Reseller]) >1'
...
--select * from stg.TouchPassSalesAmount
