...
CREATE   PROCEDURE [tm].[rpt_Get_Adherence_Covid]

@startdate date = '3/29/2020',  @enddate date = NULL

as
--/*
--subselect only columns needed for adherence test of COVID schedule adjustments
...
-- Operation:   is Scheduled daily to fill rpt.Adherence_Covid until 8/31/2020
--				
--				SQL Agent Job: Maintain Source Data - TM - Adh Covid19
--				SQL Agent Job Schedule:	Daily 6am
--				Agent Job Statement: exec tm.[rpt_Get_Adherence_Covid] '3/29/2020', NULL
--
-- Reporting:   select * from rpt.Adherence_Covid
...
if (select count(*) from sys.tables where name = 'Adherence_Covid') > 0
BEGIN
DROP TABLE rpt.Adherence_Covid
END
...
,case when time_dec < 6 THEN 1  
	WHEN time_dec >= 6 AND time_dec < 10 THEN 2  
	WHEN time_dec >= 10 AND time_dec < 13 THEN 3  
	WHEN time_dec >= 13 AND time_dec < 17 THEN 4
	WHEN time_dec >= 17 THEN 5 end as peak_period_order
,grp_date = 
		convert(varchar(12),DATEADD(wk, 0, DATEADD(DAY, 1-DATEPART(WEEKDAY, cast(the_date as date)), DATEDIFF(dd, 0, cast(the_date as date)))),101)
	+' - '
	+	convert(varchar(12),DATEADD(wk, 1, DATEADD(DAY, 0-DATEPART(WEEKDAY, cast(the_date as date)), DATEDIFF(dd, 0, cast(the_date as date)))) ,101)
into rpt.Adherence_Covid
FROM (
select o.*
