...
CREATE    PROCEDURE [process].[Eden_Extract_GJ_step2]
...
-- =============================================
-- Author:		B. Eichberger
-- Create date: 20240206
-- Description:	Process Process Eden Extract GJ for finance
...
DECLARE @isRunning INT = (
	...
	FROM
		...
		, GETDATE()) AS Elapsed, activity.stop_execution_date
			 ...
			 WHERE run_requested_date IS NOT NULL AND stop_execution_date IS NULL AND job.name='Eden Extract GJ'
		) i	
	) 
IF @isRunning = 0 
BEGIN
...
EXEC [LTD-FINANCE].msdb.dbo.sp_start_job @job_name='Eden Extract GJ'


SELECT '
As requested by '+REPLACE(@fixedUser,'LTD\','')+', the Eden GJ Transactions Processor has started.
...
The Eden GJ Transactions Processor job is alraedy running. Please come back later.
