...
CREATE   PROCEDURE [process].[part_PASSENGER_COUNT_V_OP_DOORS]
AS
...
SET @pscheme = 'schPASSENGER_COUNT_V_OP_DOORS_YRMO' ;
...
SET @sqlmaxdate = 'SELECT @maxdate = MAX([Calendar_id]) FROM [rpt].[PASSENGER_COUNT_V_OP_DOORS]' ; 

EXEC sp_executesql @sqlmaxdate
 , N'@maxdate NUMERIC(10,0) OUTPUT'
 , @maxdate = @maxdate OUTPUT ;

SELECT @currentpartition = $PARTITION.[pfPASSENGER_COUNT_V_OP_DOORS_YRMO](@maxdate) ;
...
ELSE 
 ...
 SET @sql = 'ALTER PARTITION SCHEME schPASSENGER_COUNT_V_OP_DOORS_YRMO
 NEXT USED [PRIMARY] 
 
 ALTER PARTITION FUNCTION pfPASSENGER_COUNT_V_OP_DOORS_YRMO() SPLIT RANGE (''' + CONVERT( VARCHAR(10), @partvalue, 112 ) + ''')' ;  --Range Partition Value
