...
CREATE   PROCEDURE [efare].[UMODataforImport_PS]
@yyyymm varchar(32) ,
...
specifically not for non-profit.

grant execute on [efare].UMODataforImport_PS  to rpt_reader

AUTHOR:  B. EICHBERGER
CREATED: 20200406
TICKET:  RID 17212

example: exec [efare].UMODataforImport '202306',0
...
--TEST: DECLARE @yyyymm INT = 202306, @nonprofitDiscount DECIMAL(3, 3) = 0
...
---------- TEST
--DECLARE @yyyymm varchar(32) = 202003
--DECLARE @nonprofitDiscount DECIMAL(3, 3) = .75
---------- TEST


if (select count(*) from tempdb.sys.tables where name like '%PSTPsalesDetail_PS%') <>0
BEGIN
DROP TABLE #PSTPsalesDetail_PS
END

if (select count(*) from tempdb.sys.tables where name like '%PSTPdiscLines_PS%') <>0
BEGIN
DROP TABLE #PSTPdiscLines_PS
END

if (select count(*) from tempdb.sys.tables where name like '%PSTPsalesDetail_PS%') =0
BEGIN
CREATE TABLE #PSTPsalesDetail_PS(
...
if (select count(*) from tempdb.sys.tables where name like '%PSTPdiscLine%') =0
BEGIN
CREATE TABLE #PSTPdiscLines_PS(
	[cust_no] [varchar](4) NULL,
...
FROM (
	...
	where resellerShortName like 'PS%'
...
,FORMAT(EOMONTH(cast(YYYYMM + '01' AS DATE)),'MMM') + ' ' + cast(YEAR(EOMONTH(cast(YYYYMM + '01' AS DATE))) AS VARCHAR(12)) + ' ' + LEFT(Reseller, 2)
	...
	---- test
	--DECLARE @yyyymm INT = 202307, @nonprofitDiscount DECIMAL(3, 3) = 0
	----

if (select count(*) from tempdb.sys.tables where name like '%importPS_EDEN_fileData_PS%') <>0 
BEGIN
DROP TABLE #importPS_EDEN_fileData_PS
END

if (select count(*) from tempdb.sys.tables where name like '%importPS_EDEN_fileData_PS%') = 0 
BEGIN
CREATE TABLE #importPS_EDEN_fileData_PS(
	[cust_no] [varchar](4) NULL,
	[inv_no] [varchar](36) NULL,
	[Dt1] [varchar](10) NULL,
	[Service_Type] [nvarchar](46) NULL,
...
INSERT -- select * from 
#importPS_EDEN_fileData_PS  ( [cust_no]
      ,[inv_no]
      ,[Dt1]
      ,[Service_Type]
      ,[line_no]
...
select cust_no
     ...
     , gcpro
     , RevCode
     , acctcode from #PSTPdiscLines_PS
) o
order by 2,1,17
...
select [cust_no]
      ...
      +'|'+cast([acctcode] as varchar(255)) as outputline from #importPS_EDEN_fileData_PS 
