...
CREATE   PROCEDURE [tm].[merge_ONTIME_PERFORMANCE_wo_eugTU_ssG]
AS
/*-----------LTD_GLOSSARY---------------
 created by	:  b. eichberger
 created dt	:  2025-07-03
 purpose	:  merge tm.ONTIME_PERFORMANCE_wo_eugTU_ssG from ltd-tmdata.tmmain.dbo.ONTIME_PERFORMANCE_wo_eugTU_ssG
			   for data excluding bays T and U at Eugene Station and excluding Bay G at Springfield Station

 use		:  exec [tm].[merge_ONTIME_PERFORMANCE_wo_eugTU_ssG]
...
SELECT * 
INTO #ot60
FROM ltd_dw.tm.[ontime_performance_v_wo_eugTU_ssG]
WHERE the_date >= DATEADD(DAY, -60, GETDATE())

MERGE ltd_dw.tm.ONTIME_PERFORMANCE_wo_eugTU_ssG AS t
USING #ot60 AS s
ON (t.svc = s.svc
...
DECLARE @prg VARCHAR(90) = @@SERVERNAME + '.ltd_dw.tm.merge_ONTIME_PERFORMANCE_wo_eugTU_ssG'
...
(		[MergeCode]
           ...
           ,[MergeEndDatetime])
SELECT 'OTP',
'ltd_dw.tm.ONTIME_PERFORMANCE_wo_eugTU_ssG',
'TMDM',
@prg,
