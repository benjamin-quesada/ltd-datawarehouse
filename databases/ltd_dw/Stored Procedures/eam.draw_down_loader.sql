...
CREATE   PROCEDURE [eam].[draw_down_loader]
AS

	/*
CREATED BY:		B. Eichberger
CREATED ON:		20230117
PURPOSE   :		Populate a table for a report ("Draw Down Parts and Labor")
				for year end grant draw downs. Using queries from EAM_MODEL
				to keep consistency across reporting objects.

				Probably schedule a monthly refresh.
				Data should be made incremental at some point.

MODIFIED DATE	MODIFIIED BY		DESCRIPTION
20230705		Pamela Mahan		Per Eric, Added Comments and Reason for Repair to table
20230816		Pamela Mahan		Per Matt, Added to filter "ISNUMERIC(CLASS_class_maint) = 1 OR CLASS_class_maint LIKE '%ALLFLEET%'" to allow for ALLFLEET records	
20230907		Pamela Mahan		Changed employee information from PDS to EAM, changed account id to pull from job instead of labor table
20240814		B. Eichberger		Add YES when marked as warranty on EITHER of task or work order/job

exec eam.draw_down_loader

select * from [eam].[draw_down] where work_order_no = 7631
...
DROP TABLE if exists #eamlabor
DROP TABLE if exists #empl
DROP TABLE if exists #jobmain
...
INTO #empl
FROM [LTD-EAM].[proto].[emsdba].[EMP_MAIN]
WHERE active = 'Y'
OPTION(MAXDOP 2) ;
...
LEFT JOIN [LTD-EAM].proto.[emsdba].[TSK_MAIN] t ON t.TASK_task_code = m.TASK_task_code AND t.work_order_yr = m.work_order_yr AND t.work_order_no = m.work_order_no
...
INTO	#jobmain
FROM	[LTD-EAM].proto.emsdba.JOB_MAIN WITH(NOLOCK)
...
SELECT [RepairReasonID]
		,[Description]
INTO #reason
FROM [LTD-EAM].[proto].[emsdba].[QRepairReason] WITH(NOLOCK)
OPTION(MAXDOP 2) ;	
...
--SELECT * FROM #parts WHERE parts_total_cost <> total_cost_minus_warranty


TRUNCATE TABLE [eam].[draw_down]
	
INSERT INTO [eam].[draw_down]
           ([rn]
           ,[work_order_yr]
           ,[work_order_no]
           ,[work_order_yr_no]
	...
	(	SELECT	rn = ROW_NUMBER() OVER (PARTITION BY l.work_order_yr
			   ...
			   ,j.comment_area
		FROM	#eamlabor l
		LEFT JOIN #jobmain j WITH(NOLOCK)ON j.work_order_yr = l.work_order_yr
											AND j.work_order_no = l.work_order_no
		LEFT JOIN #empl jc WITH(NOLOCK)ON CAST(jc.employee_id AS VARCHAR(50)) COLLATE SQL_Latin1_General_CP1_CI_AS = j.usr_closed_by COLLATE SQL_Latin1_General_CP1_CI_AS
		LEFT JOIN #empl jf WITH(NOLOCK)ON CAST(jf.employee_id AS VARCHAR(50)) COLLATE SQL_Latin1_General_CP1_CI_AS = j.usr_finished_by COLLATE SQL_Latin1_General_CP1_CI_AS
		LEFT JOIN #reason r ON r.[RepairReasonID] = j.REAS_reas_for_repair
		) u
		LEFT JOIN #parts p ON p.work_order_yr_no = CAST(u.work_order_yr AS VARCHAR(32)) + '-' + CAST(u.work_order_no AS VARCHAR(32))
