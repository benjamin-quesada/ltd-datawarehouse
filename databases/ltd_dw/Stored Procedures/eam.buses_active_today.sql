...
CREATE   PROCEDURE [eam].[buses_active_today] AS
...
USE			: exec [eam].[buses_active_today]

CHANGED ON	: 20240322
CHANGE BY	: B. Eichberger
Purpose		: Remove reliance on table functions
...
DROP TABLE IF EXISTS #cte_mrf_dt
DROP TABLE IF EXISTS #cte_mrf_q
DROP TABLE IF EXISTS #lastftk
DROP TABLE IF EXISTS #busAct
DROP TABLE IF EXISTS #cte_mrf_d
...
SELECT eq_equip_no AS bus,max_fuel
	...
	INTO -- SELECT * FROM 
	#ftk
	FROM [ltd-eam].ltd_db.dbo.fuel_tickets WITH (NOLOCK) 
              where  fuel_type IN('uls')

	SELECT ft.bus 
    ,MAX(ft.ftk_date) mr_datetime
	INTO #cte_mrf_dt
        FROM #ftk  ft
        JOIN tm.t_buses_active_today bt ON bt.bus COLLATE DATABASE_DEFAULT = ft.bus
    WHERE ft.fuel_type IN('uls')
    GROUP BY ft.bus

	SELECT bus,mr_datetime
                INTO #cte_mrf_d
				FROM #ftk

	SELECT d.bus,CAST(d.mr_datetime AS DATE) mr_date,SUM(ft.ftk_fuel_qty) mr_qty
			INTO #cte_mrf_q
			FROM #cte_mrf_d d
            JOIN #ftk ft ON ft.bus = d.bus AND ft.mr_datetime = d.mr_datetime
            WHERE ft.fuel_type = 'uls'
			GROUP BY d.bus,d.mr_datetime

SELECT [bus]                  = q.bus
      ,[mr_fueling_date]      = q.mr_date
	  ,[mr_fueling_datetime]  = dt.mr_datetime
	  ,[mr_fueling_age_hours] = DATEDIFF(HOUR, dt.mr_datetime, GETDATE())
      ,[mr_fueling_date_qty]  = CAST(ROUND(q.mr_qty, 1) AS NUMERIC(5,1))
INTO #lastftk
  FROM #cte_mrf_q  q
  JOIN #cte_mrf_dt dt ON dt.bus = q.bus
...
,[rpt_order_by_bus] = RIGHT('00000' + CAST(tb.class AS VARCHAR(5)), 5) + RIGHT('00000' + tb.bus, 5)
,[rpt_order_by_at_ltd_and_pullin] = CASE tb.at_ltd WHEN 1 THEN 'a' ELSE 'z' END + tb.pull_in + RIGHT('00000' + CAST(tb.class AS VARCHAR(5)), 5) + RIGHT('00000' + tb.bus, 5)
,[rpt_order_by_block] = RIGHT('0000' + CAST(tb.last_block AS VARCHAR(4)), 4) -- select * 
...
WHERE tb.miles_total_est IS NOT NULL AND tb.last_block IS NOT NULL 
OPTION (MAXDOP 2)
