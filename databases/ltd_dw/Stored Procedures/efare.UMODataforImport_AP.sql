...
CREATE   PROCEDURE [efare].[UMODataforImport_AP] 
@yyyymm varchar(32) ,
@nonprofitDiscount DECIMAL(3, 3) = 0
AS

/*
output files formatted for import to EDEN
specifically for Annual Pass.

grant execute on [efare].[UMODataforImport_AP] to public

AUTHOR:  B. EICHBERGER
CREATED: 20230406
TICKET:  RID 17212

example: exec [efare].[UMODataforImport_AP]  '202303',0
...
--DECLARE @yyyymm INT = 202301, @nonprofitDiscount DECIMAL(3, 3) = 0
...
---------- TEST
--DECLARE @yyyymm varchar(32) = 202301
--DECLARE @nonprofitDiscount DECIMAL(3, 3) = .75
---------- TEST


if (select count(*) from tempdb.sys.tables where name like '%APTPsalesDetail_AP%') <>0
BEGIN
DROP TABLE #APsalesDetail_AP
END

if (select count(*) from tempdb.sys.tables where name like '%APTPdiscLines_AP%') <>0
BEGIN
DROP TABLE #APdiscLines_AP
END

if (select count(*) from tempdb.sys.tables where name like '%APTPsalesDetail_AP%') =0
BEGIN
CREATE TABLE #APsalesDetail_AP(
...
if (select count(*) from tempdb.sys.tables where name like '%APTPdiscLine%') =0
BEGIN
CREATE TABLE #APdiscLines_AP(
	[cust_no] [varchar](4) NULL,
...
SELECT YYYYMM
	,lineCode
	,Reseller = CASE 
					WHEN CHARINDEX('|', Reseller) <> 0 
						THEN SUBSTRING(Reseller, 1,CHARINDEX('|', Reseller))
					WHEN (CHARINDEX('|', Reseller) = 0 AND TRY_CAST(RIGHT(Reseller, 4) as int) IS NOT NULL) 
						THEN LEFT(Reseller, LEN(Reseller) - CHARINDEX(' ',REVERSE(Reseller)))+'|'
					ELSE Reseller
				END 

	,cust_no = CASE 
					WHEN CHARINDEX('|', Reseller) <> 0 
						THEN Right(Reseller, CHARINDEX('|',REVERSE(Reseller))-1)
					WHEN (CHARINDEX('|', Reseller) = 0 AND TRY_CAST(RIGHT(Reseller, 4) as int) IS NOT NULL) 
						THEN Right(Reseller, CHARINDEX(' ',REVERSE(Reseller))-1)
					ELSE Reseller
				END
	...
	CASE WHEN reseller LIKE 'AP%' 
						 AND rtrim(ltrim(Replace(Replace(Product, ' Count', ''), '  Count', '') )) not like '%card%'
						 THEN @nonprofitDiscount
						 ELSE 0 END 
	,OriginalLineTotal = (AmtSales)
	,InvoiceLineTotal = CASE WHEN reseller LIKE 'AP%' 
			AND rtrim(ltrim(Replace(Replace(Product, ' Count', ''), '  Count', '') )) not like '%card%'
			THEN (AmtSales - (AmtSales * @nonprofitDiscount)) ELSE (AmtSales) END
FROM (
		SELECT replace(SaleYYYYMM,'-','') YYYYMM
	,RTRIM(LTRIM(LEFT([shortName],3))) CustomerGroup
		,left(SaleYYYYMM, 4) AS TxYr
		,right(SaleYYYYMM, 2) AS TxMo
		,right(cast(left(SaleYYYYMM, 4) AS VARCHAR(32)), 2) + right('00' + cast(right(SaleYYYYMM, 2) AS VARCHAR(32)), 2) AS lineCode
		,SUM(cost) [AmtSales]
		,SUM(qty) [CountSales]
		--,Pricing = AmtSales
		,faretype AS Product
		,resellerShortName AS reseller	
		,eomonth(right(SaleYYYYMM, 2) + '/01/' + cast(right(SaleYYYYMM, 2) as varchar(3))) YearMonthBilled
	from [ltd_dw].[efare].[SalesbyResellerv2]
	where resellerShortName like 'AP%'
	and FareType not like '%new card%' and FareType not like '%replaced card%'
	AND replace(SaleYYYYMM,'-','') = @YYYYMM
	GROUP BY 
	 replace(SaleYYYYMM,'-','')
		,left(SaleYYYYMM, 4) 
		,right(SaleYYYYMM, 2) 
		,right(cast(left(SaleYYYYMM, 4) AS VARCHAR(32)), 2) + right('00' + cast(right(SaleYYYYMM, 2) AS VARCHAR(32)), 2) 
		,FareType
		,resellerShortName
		,eomonth(right(SaleYYYYMM, 2) + '/01/' + cast(right(SaleYYYYMM, 2) AS VARCHAR(3)))
	,ShortName	) o 

	

INSERT #APdiscLines_AP (
	   [cust_no]
      ,[inv_no]
      ,[Dt1]
      ,[Service_Type]
	...
	,LineCategory = 'tpacct'
	,[Description] = 'PS Discount '+ cast(cast((@nonprofitDiscount*100) as INT) as varchar(12)) + '%' 
	,' ' AS noNote
...
if (select count(*) from tempdb.sys.tables where name like '%import_EDEN_fileData_AP%') <>0 
BEGIN
DROP TABLE #import_EDEN_fileData_AP
END

if (select count(*) from tempdb.sys.tables where name like '%import_EDEN_fileData_AP%') = 0 
BEGIN
CREATE TABLE #import_EDEN_fileData_AP(
	[cust_no] [varchar](4) NULL,
	[inv_no] [varchar](36) NULL,
	[Dt1] [varchar](10) NULL,
	[Service_Type] [nvarchar](46) NULL,
...
INSERT -- select * from 
#import_EDEN_fileData_AP  ( [cust_no]
      ,[inv_no]
      ,[Dt1]
      ,[Service_Type]
      ,[line_no]
...
INSERT #import_EDEN_fileData_AP  ( [cust_no]
      ,[inv_no]
      ,[Dt1]
      ,[Service_Type]
      ,[line_no]
...
select cust_no,inv_no,Dt1,Service_Type,line_no = row_number() OVER (PARTITION BY Cust_No ORDER BY AmtTtl Desc)
,LineCategory,[Description],noNote,AmtTtl,Dt2,Qty,'',Rate,'',gcpro,RevCode,acctcode
from (
	SELECT cust_no
	,cast(YYYYMM AS VARCHAR(32)) + cust_no AS inv_no
	,Dt1 = right('00' + cast(YYYYMM as varchar(12)),2 ) +'/'
...
select [cust_no]
      ...
      +'|'+cast([acctcode] as varchar(255)) as outputline from #import_EDEN_fileData_AP 
