...
CREATE   PROCEDURE [rpt].[TouchPassDataMakeReady_v3]
as
set NOCOUNT on
...
TICKET:  RID 16146 - Annual Pass Enabled - see rpt.TouchPassDataforImport_PS_and_AP

exec [rpt].[TouchPassDataMakeReady_v3]
...
exec rpt.TouchPassBillingAmount_v3
exec rpt.TouchPassBillingCount_v3
...
DELETE FROM rpt.TouchPassSales WHERE Reseller LIKE 'AP%' AND Reseller NOT LIKE '%|%'
DELETE FROM stg.TouchPassSalesAmount WHERE Reseller LIKE 'AP%' AND Reseller NOT LIKE '%|%'
DELETE FROM stg.TouchPassSalesCount WHERE Reseller LIKE 'AP%' AND Reseller NOT LIKE '%|%'
...
             ,@recipients = 'nooshi.ghasedi@ltd.org;ar@ltd.org;' --'barb.eichberger@ltd.org' --
			 ,@copy_recipients = 'barb.eichberger@ltd.org;pamela.mahan@ltd.org;'
