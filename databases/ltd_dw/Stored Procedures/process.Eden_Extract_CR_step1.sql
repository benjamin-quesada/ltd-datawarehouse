...
CREATE   PROCEDURE [process].[Eden_Extract_CR_step1]  @currentUser VARCHAR(42)
...
-- Description:	Process Eden Extract CR for finance
-- example:	exec process.Eden_Extract_CR @currentUser	
...
SELECT SUM(rc) FROM 	
	...
	FROM  
		...
		, GETDATE()) AS Elapsed, activity.stop_execution_date
			 ...
			 WHERE activity.run_requested_date IS NOT NULL AND activity.stop_execution_date IS NULL AND job.name='Eden Extract CR') i
		 ) t 
	)
IF @isRunning = 0 
BEGIN
	...
	VALUES ('Eden Extract CR','UserFromPBIRS', @fixedUser
	);
...
Click here to start Processing Eden Extract CR.
...
The Eden Extract CR job is running right now. Please come back later.
 '
 AS banner
END 
