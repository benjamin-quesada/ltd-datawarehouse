...
CREATE   PROCEDURE [efare].[GET_SALES]
...
where fileloading = '''+ @currentfile + ''' and not exists (select TxId from [efare].[SALE] where TxId = s.TxId)
group by 
[TxId]
      ,[Ts]
      ,[Type]
...
insert [process].[Fileload] (
      ...
      ,[FileSourceGroup]
      ,[FileRowCount])
 select fileloaded,''eFARE'',count(*) from ##OutputTbl0142 group by fileloaded
