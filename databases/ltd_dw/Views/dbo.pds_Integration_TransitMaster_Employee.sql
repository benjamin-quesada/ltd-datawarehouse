...
CREATE VIEW [dbo].[pds_Integration_TransitMaster_Employee] 
AS

/*
AUTHOR   : BEichberger
DATE     : 20210330
PURPOSE  : Data Extract from PDS Vista Sources to populate the TranstMaster
		   TMMAIN database OPERATORS tables.
DATE	 : 20210428
PURPOSE  : ADDED PHONE TYPE FOR PRIMARY PHONE - TM.TELEPHONE DIRECTORY
...
,CASE WHEN LEN(LTRIM(RTRIM(ISNULL(p2.last_name,'') + ', ' + ISNULL(p2.first_name,'')))) < 4 THEN NULL ELSE 
		LTRIM(RTRIM(ISNULL(p2.last_name,'') + ', ' + ISNULL(p2.first_name,''))) END AS lksup_supervis
,CASE WHEN LEN(RTRIM(LTRIM(RTRIM(ISNULL(p2.last_name,'') + ', ' + ISNULL(p2.first_name,''))) + ' ' + LTRIM(RTRIM(ISNULL(p2.middle_name,''))))) < 4 THEN NULL ELSE 
		RTRIM(LTRIM(RTRIM(ISNULL(p2.last_name,'') + ', ' + ISNULL(p2.first_name,''))) + ' ' + LTRIM(RTRIM(ISNULL(p2.middle_name,'')))) END AS lksup_supervis_full
...
SELECT * FROM (
SELECT 
rn = ROW_NUMBER() OVER (PARTITION BY o.person_id ORDER BY o.person_id),
...
seniority_date				= CAST(o.[seniority_date] AS DATE) ,
p_fname						= o.[first_name] ,
p_mi						= o.middle_name ,
p_lname						= o.last_name ,
full_name					= o.fullname ,
p_nickname					= RTRIM(LTRIM(o.aka)) ,
...
phone						= CASE WHEN [currently_active] = 'Y' THEN COALESCE(primary_phone,cell_phone) ELSE NULL END,
phone_code					= prim_phone_type ,
...
p_supervis					= COALESCE(CASE WHEN LEN(LTRIM(RTRIM(ISNULL(p2.last_name,'') + ', ' + ISNULL(p2.first_name,'')))) < 4 THEN NULL ELSE 
									LTRIM(RTRIM(ISNULL(p2.last_name,'') + ', ' + ISNULL(p2.first_name,''))) END ,s.lksup_supervis),
p_supervis_full				= COALESCE(CASE WHEN LEN(RTRIM(LTRIM(RTRIM(ISNULL(p2.last_name,'') + ', ' + ISNULL(p2.first_name,''))) + ' ' + LTRIM(RTRIM(ISNULL(p2.middle_name,''))))) < 4 THEN NULL ELSE 
									RTRIM(LTRIM(RTRIM(ISNULL(p2.last_name,'') + ', ' + ISNULL(p2.first_name,''))) + ' ' + LTRIM(RTRIM(ISNULL(p2.middle_name,'')))) END,s.lksup_supervis_full),
p_supv_active				= p2.hr_is_active,
empno_supervisor			= p2.employee_id,
p_orighire					= [earliest_hire_date] ,
[most_recent_hire_date] ,
[most_recent_term_date] ,
[earliest_hire_date]
FROM (
SELECT p.person_id,
p.employee_id AS [employee_number]
	  ...
	  ,p.fullname
	  ,p.aka
	  ,f.phone_number AS primary_phone
	  ...
	  ,f.phone_code AS prim_phone_type
